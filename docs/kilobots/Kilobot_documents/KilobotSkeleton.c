...
static volatile uint8_t incoming_message[5];
...
//inline function macro for updating the next message to be transmitted (note: does not actually transmit message)
...
int main(void)
{
	...
	{	
		...
		{	
			...
			//set motors set_motor(cw_motor,ccw_motor) values between 0(off) and 255 (full on)
			//set_motor(a,b);
			
			//busy wait for x ms, interrupts can still trigger ...._delay_ms(x), max value 4000
			//_delay_ms(200);

			//set led color set_color(red,green,blue) values can be from 0(off)-7(brightest)
			//set_color(1,1,1);

			//print int.  be carefull, can effect timing
			//kprinti(int);

			//print string up to 10 characters. be carefull, can effect timing
			//kprints("helloworld");

			//set message values to be sent every .2 seconds, 2 bytes tx0,tx1
			//message_out(tx0,tx1);

			//take oldest message off of rx buffer
			//first call get_message();
			//next check if message_rx[5]==1 
			//if so, message is in message_rx[0],message_rx[1],message_rx[2]
			//distance from that transmission is in message_rx[3]  
			//get_message();
			//if(message_rx[5]==1)
			//data=message_rx[0]
			...
			//for example if it returned 394, then voltage is 3.94 volts
			//measure_voltage( );
			...
			//get_ambient_light();
			//gets the value of ambient light. 
			//note: will return -1 if there is an incoming message (which also uses a/d)
			//note: turns off interrupts for a bit to sample a/d
